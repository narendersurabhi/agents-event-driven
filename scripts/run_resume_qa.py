...
def main():
    p = ArgumentParser(description="Run resume QA against existing JD/profile/tailored JSON outputs.")
    p.add_argument("--jd-json", type=Path, required=True, help="Path to JDAnalysisResult JSON (e.g., out/jd.json)")
    p.add_argument("--profile-json", type=Path, required=True, help="Path to ProfessionalProfile JSON (e.g., out/profile.json)")
    p.add_argument("--tailored-json", type=Path, required=True, help="Path to TailoredResume JSON (e.g., out/tailored.json)")
    ...
    p.add_argument("--log-file", type=Path, help="Optional path for structured logs; defaults to repo logs directory")
    args = p.parse_args()
