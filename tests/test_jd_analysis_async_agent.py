...
class FakeAsyncLLMGood:
    async def chat(
        self, messages: list[dict[str, str]], model: str, temperature: float = 0.0
    ) -> str:
        return """
        {
...
async def test_async_analyze_happy_path() -> None:
    agent = AsyncJDAnalysisAgent(llm=FakeAsyncLLMGood())
...
async def test_async_analyze_invalid_json_raises() -> None:
    agent = AsyncJDAnalysisAgent(llm=FakeAsyncLLMBadJSON())
